...
def find_asset_type(asset_name,asset_types):
  logging.debug("FIND ASSET TYPES:")
...
def find_latest_version(file_descriptor,folder_path):
  logging.debug("FIND LATEST VERSION:")
...
def getAnimationVersion(json_filepath):
  ...
  logging.debug("aniamtion file version: " + str(animation_file_version))
...
def import_assets(json_file_path,shading_descriptor):
  ...
  for asset in json_data["scene"]:
    ...
    logging.debug("asset_shading_basename: " + asset_shading_basename)
    ...
    logging.debug("copy command: "+copy_command)
    ...
    for cache in json_data["scene"][asset]:
      ...
      logging.debug("cache_name: " + cache_name)
    ...
    pm.importFile(temp_shading_scene,force=True)

    #delete temp shading scene
    try:
          os.remove(temp_shading_scene)
          logging.debug("deleted file: " + temp_shading_scene)
...
def open_renderTemplate(json_filepath,renderTemplate_descriptor,assembly_descriptor,animationFile_version):
  ...
  assembly_filepath = find_latest_version(assembly_descriptor,assembly_root_path)
...
def set_frame_range(json_filepath):
  ...
  logging.debug("setting frame range to: {} {}".format(startFrame,endFrame))
...
camera_filepath = import_camera(json_filepath,camera_descriptor) 
...
with open(json_filepath) as json_file:
  ...
  json_data['camera'] = camera_filepath
